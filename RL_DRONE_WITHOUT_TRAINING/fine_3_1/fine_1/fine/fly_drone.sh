...
SESSION="fly_drone"
VENV_ACTIVATE="/home/c3ilab/Desktop/rl_mini_pro/final_rl_minipro/rl_atom_bomb/bin/activate"  # Adjust path if needed
...
tmux send-keys -t "$SESSION":0 "source ${VENV_ACTIVATE} && __NV_PRIME_RENDER_OFFLOAD=1 __GLX_VENDOR_LIBRARY_NAME=nvidia make -C /home/c3ilab/PX4-Autopilot px4_sitl gz_x500_mono_cam" C-m
...
tmux send-keys -t "$SESSION":3 "source ${VENV_ACTIVATE} && sleep 15 && python3 /home/c3ilab/Desktop/fine_3_1/fine_3_1/fine_1/fine/send_velocity.py" C-m

# 5. Set OFFBOARD mode: Create a new window and call the service after delay.
tmux new-window -t "$SESSION" -n 'OFFBOARD_Mode'
tmux send-keys -t "$SESSION":4 "source ${VENV_ACTIVATE} && sleep 17 && ros2 service call /mavros/set_mode mavros_msgs/srv/SetMode \"{custom_mode: 'OFFBOARD'}\"" C-m

# 6. Arm Drone: Create a new window to arm the drone.
tmux new-window -t "$SESSION" -n 'Arming'
tmux send-keys -t "$SESSION":5 "source ${VENV_ACTIVATE} && sleep 23 && ros2 service call /mavros/cmd/arming mavros_msgs/srv/CommandBool \"{value: true}\"" C-m

# 7. Run combined_left_yolo.py: Create a new window.
tmux new-window -t "$SESSION" -n 'YOLO'
tmux send-keys -t "$SESSION":6 "source ${VENV_ACTIVATE} && sleep 24 && python3 /home/c3ilab/Desktop/fine_3_1/fine_3_1/fine_1/fine/combined_left_yolo.py" C-m

# 8. Run collision detection: Create a new window.
tmux new-window -t "$SESSION" -n 'Collision'
tmux send-keys -t "$SESSION":7 "source ${VENV_ACTIVATE} && sleep 35 && python3 /home/c3ilab/Desktop/fine_3_1/fine_3_1/fine_1/fine/collision.py" C-m
